...
(function( $, undefined ){
	$( function(){
		...
		// Alert stubs (weather & traffic)
			if ($("#masthead .weather").length) {
				$('<li><a href="#' + $('.weather-info').attr('href').split('#')[1] + '">Weather</a></li>')
	...
	// Scoreboard switcher
		var $scores = $(".bg-scores .live-scores");
		if ($scores.length > 1) {
			// Build navigation dynamically (TK could/should this be static?)
			var $stubScoreNav = $(innerShiv('<div class="score-nav"><nav><h1>More live coverage<b>:</b></h1><ul></ul></nav></div>', false)).appendTo('.bg-scores'),
				$oNav = $stubScoreNav.find('ul'),
				sScoreClass = {
					show : "score-active",
					link : "active"
				};

			for (var i = 0, lScores = $scores.length; i < lScores; i++ ) {
				var $oStub = $($scores[i]),
					sSlug = $oStub.attr('id'),
					sLabel = $oStub.find('h2 b').text(),
					sClass = '';
				if (i === 0) {
					sClass = ' class="' + sScoreClass.link + '"';
					$("#" + sSlug)
						.addClass(sScoreClass.show);
				}
				$oNav.append('<li><a' + sClass + ' href="#' + sSlug + '">' + sLabel + '</a></li>')
			}

			var $scoreTabs = $(".score-nav a");
			$scoreTabs.click(function(e) {
				var sSlug = $(this).attr("href").split("#")[1];

				$scores
					.filter(":not(#" + sSlug + ")")
					.removeClass(sScoreClass.show);
				$("#" + sSlug)
					.addClass(sScoreClass.show);

				$scoreTabs.removeClass(sScoreClass.link);
				$(this).addClass(sScoreClass.link);
	...
	//text size switcher
		...
		$(sHtmlTextSize).insertBefore("li.my-account");
		...
		// Click handler
		$oTextSizeLinks.bind( globe.e.down, function() {
			var $oThis = $(this),
				sClass = $oThis.attr("class");

			updateSize(sClass, $oThis);
			return false;
		});
	...
	(function( $ ){
		...
		// Section nav
			...
			if (screen.width > 480 && !(globe && globe.dev && globe.dev.mobileOverride) ) {
			    /* Total placeholder; should this be loaded in via AJAX? Or presented differently? TKTK */
				...
				var loadPreviews = function() {
				   	var section    = $('h1.section').text(),
						sContainer = [
							'<div class="in-section">',
								'<h1 class="hed-section">In this section</h1>',
							'</div>'
						].join("");
					...
					if (!$(".article .in-section").length && !$("#comments").length) {
					    ...
					    $.get("/Fragment/SysConfig/WebPortal/BostonGlobe/Framework/skins/leaf/story/in_section_bottom.jpt?section=" + section_name, function(data) {
						    $(".article .in-section").append(data);
						});
					}
				    }
				}
			      ...
			      submission = function () {
			        $.ajax({
			          ...
			          success: function () {
			            ...
			            $this.find('h4').html('Your article has been sent.');
			          },
			          error: function () {
...
(function ( $ ) {
    ...
    $(function ( config ) {
            ...
            var defaults = {
                ...
                count    : $( 'meta[ name=eomportal-loid ]' ).attr( 'content' ),
                shorturl : $( '.share-twitter' ).data( 'short' ),
                counter  : 'ShareCount',
                counturl : '/Statistics'
            },

			_postToServer = function postRemoteHTML(url,data) {
				var xmlHttp;
            ...
            _updateCount = function ( curl, stats ) {
            	var url  = opt.counturl + "?counter=" + opt.counter + "&loid=" + opt.count;
            	_postToServer(url, stats);                
            },
            ...
            share = {
                ...
                facebook : function () {
                	var myUrl = encodeURIComponent(opt.url + "?s_campaign=sm_fb");
                    var furl = "http://www.facebook.com/sharer.php?u=" + myUrl + "&t=" + opt.headline;
      	      	    window.open( furl );
                    _updateCount( furl, '[1,0,0]' );
                },

                twitter : function () {
                    var turl = "http://twitter.com/intent/tweet?text=" + opt.headline + "&url=" + opt.shorturl + "&via=BostonGlobe";
      	      	    window.open( turl );
                    _updateCount( turl, '[0,1,0]' );
                },

                linkedIn : function () {
                    var lurl = "http://www.linkedin.com/shareArticle?mini=true&url=" + opt.url + "&title=" + opt.headline + "&summary=" + opt.summary + "&source=BostonGlobe";
      	      	    window.open( lurl );
                    _updateCount( lurl, '[0,0,1]' );
                }
...
(function($){
	...
	function getMeta(){
		return JSON.parse(s.getItem(metaKey)) || {}
	}
	...
	function getItems() {
		return JSON.parse(s.getItem(itemsKey)) || []
	}
	...
	function getCounts(){
		return JSON.parse(s.getItem(countsKey)) || []
	}
...
(function($){
	...
	function addSaveButtons() {
		...
		//support for two-finger touch show & one finger hide
		var touchInPlay = false,
			checkTouches = function( e ){
				if( touchInPlay ){
					setTimeout(function(){
						touchInPlay = false;
					}, 200);
				...
				if( e.originalEvent.touches.length === 2 ){
					...
					if( saveable.eq(0).parent().is( ".hover" ) ){
						saveable.parent().trigger( "hidebutton" );
						allOn = false;
					}
					else{
						saveable.parent().trigger( "showbutton" );
						allOn = true;
					}
				}
			};
	...
	globe.OAS.loader = function() {
		...
		for ( ; i < iAds; i++ ) {
			...
			if (adKeys[sPos]) {
				...
				sAdURL = globe.OAS.url + globe.OAS.sitepage + "/" + "@" + sPos + "?rid=" + iRandom;
				...
				// BELOW FOR DEVELOPMENT ONLY TKTKTK
				// $thisAd.html( '<img src="_img/sample-ad-350x200.jpg" alt="Sample Ad" />' );
			}
		}
	};
				...
				$(".facebook-like").each(function() {
					var oDiv = $(this)
						,params = {
							...
							,width	: (oDiv.data("width")) ? parseInt(oDiv.data("width")) : 140
